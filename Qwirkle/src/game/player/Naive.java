...
public class Naive extends Player implements Ai {
	...
	/**
	 * Returns the first possible move it encounters as long as the
		...
		for (long stop = System.nanoTime() 
			...
			if (!getHand().isEmpty()) {
				...
				if (board.isValidMove(91, 91, new Tile(colorFirstMove, shapeFirstMove))) {
					move = move.concat("MOVE " + colorFirstMove.getChar() 
							+ shapeFirstMove.getChar() + " " + 91 + " " + 91);
						...
						for (int row = maxy; row <= miny; row++) {
							for (int col = minx; col <= maxx; col++) {
								...
								if (board.isValidMove(row, col, new Tile(color, shape))) {
									move = move.concat(
											"MOVE " + color.getChar() + shape.getChar() + " " 
													+ row + " " + col);
