...
public class Client extends Thread {
	...
	/**
	 * Creates a client with the arguments given to it. If the arguments are
	 * incorrect it throws an error and exits the program. Otherwise it will
	 * read the command line and gives the input to the sendMessage method.
	 * 
	 * @param args
	 *            String[]
	 */
	...
	/**
	 * Reads the output of the server. In the case "WELCOME" it will create a
	 * board and a player on the client side. If the playerName is "Naive" it
	 * will create a Naive AI which will make the moves instead of the
	 * LocalPlayer. In the case "NAMES" it will print out who takes part in the
	 * game and adjusts the jar according to the number of players. In the case
	 * "NEXT" it will print out "It's your turn!" if the player number given
	 * equals the your player number. If the player currently playing is an
	 * instance of Naive it will determine his move and send it back to the
	 * server. In the case "NEW" it will add the given tiles to your hand and
	 * will print: "There are no more tiles in the jar left" If the jar is
	 * empty. It will also adjust the virtualJar accordingly. In the case "TURN"
	 * it will modify the board according to the move that comes after TURN. It
	 * will also print the board and the hand of the player for the UI. In the
	 * case "KICK" it will print out the message if the LocalPlayer is the one
	 * that got kicked, or adjust the virtualJar if another player was kicked.
	 * In the case "WINNER" it will print out the output of the server stating
	 * who won the game.
	 */
	public void run() {
		while (readIt) {
				...
				if (input != null) {
					...
					switch (splittedInput[0]) {
						...
						case "KICK":
							try {
								if (Integer.parseInt(
										  splittedInput[1]) == player.getPlayerNumber()) {
									tui.showKick(Integer.parseInt(
											  splittedInput[1]), input.substring(9));
									return;
								} else {
									tui.showKick(Integer.parseInt(
											  splittedInput[1]), input.substring(9));
	...
	/**
	 * Reads the message of the message given to the method. In the case of
	 * "MOVE" it will first check if the move is valid, if it is it will modify
	 * the board and the hand of the player according to the move, and send the
	 * message to the server. If it is not a valid move, it will not send the
	 * move to the server, but instead print out that the move is invalid and
	 * therefore the player should make a different move. In the case of "SWAP"
	 * it checks if the virtualJar is empty, if that is the case it will print
	 * out that the jar doesn't have the amount of tiles the player wants to
	 * swap. Otherwise it will send the server the message. In the case of
	 * "HELLO" it will send the message to the server. In the case of "JAR" it
	 * will print how many tiles are left in the virtualJar. In the case of
	 * "HINT" it will give you a valid move based on the players hand and the
	 * board, and "No options left" if you can't make a move. Default this
	 * method will print "Not a valid command" because the user didn't enter a
	...
	/**
	 * This method gives a valid move or no options left if the player can't
	 * make any more moves.
	 * 
	 * @param givenPlayer
	 * @param givenBoard
	...
	public String checkForMoves(Player givenPlayer, Board givenBoard) {
		...
		Player deepPlayer = new HumanPlayer(
				  givenBoard, givenPlayer.getName(), givenPlayer.getPlayerNumber());
		deepPlayer.getHand().addAll(givenPlayer.getHand());
